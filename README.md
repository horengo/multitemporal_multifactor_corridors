...
1. Maps of areas with loose sand surfaces (i.e. deserts). This is not generated by the notebook code as it requires a Machine Learning multi-sensor approach that would have been too costly.
   Consequently, the notebook and calls the resulting map from the author's GEE asset repository. The GEE code for the generation of this map has also been provided for reproducibility reasons but it is not required to
   run the notebook or the GEE code for the generation of cost surfaces.
